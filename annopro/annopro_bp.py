...
class Ontology(object):
    def __init__(self, filename='/public/home/zhengqq/data/Train/go-basic.obo', with_rels=False):
...
def diamond_score(diamond_scores_file, label, data_path,term_path):
    with open("/public/home/zhengqq/data/Train/go.pkl", 'rb') as file:
        go = pickle.loads(file.read())
    train_df = pd.read_pickle("/public/home/zhengqq/data/Train/train_dic.pkl")
    test_df = data_path
    annotations = train_df['Annotation'].values
    annotations = list(map(lambda x: set(x), annotations))
    prot_index = {}
    for i, row in enumerate(train_df.itertuples()):
        prot_index[row.Protein] = i
    diamond_scores = {}
    ...
    for i in range(0, len(label)):
        ...
        for j in range(0, len(label[0])):
            ...
            # print(go_id)
            if go_id == 'GO:0071427':
                label[i, j] = label[i, j]*(1 - alphas['biological_process'])
            elif go_id == 'GO:0099061':
                label[i, j] = label[i, j]*(1 - alphas['cellular_component'])
            else:
...
def init_evaluate(data_size, batch_size, model_file, data_path, term_path):
    ...
    # preds=diamond_score("/public/home/zhengqq/data/fanxiu/valid.txt",preds, data_path,term_path) #25个小数据集
    preds=diamond_score("/public/home/zhengqq/data/fanxiu/GB/20421pkl/20421.txt",preds, data_path,term_path) #human总数据集
    return terms, labels, preds
...
def evaluate(model_file, data_path, data_size=8000, batch_size=16,
    ...
    precision_t=precisions[alpha]
    recall_t=recalls[alpha]
    sorted_index = np.argsort(recalls)
    recalls = recalls[sorted_index]
    precisions = precisions[sorted_index]
    ...
    evaluate_info += f'\t, {len(terms)}: fmax={fmax:0.3f}, mcc={mcc:0.3f}, roc_auc={roc_auc:0.3f}, precision={precisions[alpha]:0.3f}, recall={recalls[alpha]:0.3f}, threshold={alpha}, AUPR={AUPR}\n'
    # plt.show()
    # aucli = []
    # fs = []
    # with open(term_path, 'rb') as file:
...
def train_model(hparams):
    ...
    model = load_model(hparams.model_file)
    #     model.summary()
    checkpointer = ModelCheckpoint(filepath=hparams.model_file, verbose=1, save_best_only=True)
    earlystopper = EarlyStopping(monitor='val_loss', patience=3, verbose=1)
    tbCallBack = TensorBoard(log_dir="./model", histogram_freq=1, write_grads=True)
    logger = CSVLogger('/public/home/zhengqq/data/log/reCRNN_bi.log')
    terms_df = pd.read_pickle(hparams.term_file)
    terms = terms_df.terms.values.flatten()
    terms_dict = {v: i for i, v in enumerate(terms)}
    nb_classes = len(terms)
    t = open(hparams.data_path, 'rb')
    data_df = pickle.load(t)
    # t.close()
    if len(data_df) > hparams.data_size:
        data_df = data_df.sample(n=hparams.data_size, random_state=918)

    if hparams.valid_path == 'none':
        valid_df = data_df.sample(frac=0.2, random_state=918)
        train_df = data_df[~data_df.index.isin(valid_df.index)]
    else:
        train_df = data_df
        v = open(hparams.valid_path, 'rb')
        valid_df = pickle.load(v)
    # valid_steps = int(math.ceil(len(valid_df) / hparams.batch_size))
    # train_steps = int(math.ceil(len(train_df) / hparams.batch_size))
    # train_generator = DFGenerator(train_df, terms_dict, nb_classes, hparams.batch_size)
    # valid_generator = DFGenerator(valid_df, terms_dict, nb_classes, hparams.batch_size)
    # #     训练模型
    # if hparams.early:
    #     his = model.fit(
    #         train_generator,
    ...
    #         max_queue_size=hparams.batch_size,
    #         workers=12,
    ...
    evaluate(hparams.model_file, valid_df, data_size=10000, batch_size=32, term_path=hparams.term_file)
...
def pretrain_model(hparams):
    ...
    # protein_similary
    inp_similary = Input(shape=(92120), dtype=np.float32)
    ...
    model.save(hparams.model_file)
    model.summary()


def CRNN_model(hparams):
    terms_df = pd.read_pickle(hparams.term_file)
    ...
    nb_classes = len(terms)
    inp_hot = Input(shape=(39, 39, 7), dtype=np.float32)
    cnn1 = Conv2D(64, (3, 3), activation='relu', input_shape=(39, 39, 7),trainable=hparams.frozen)(inp_hot)
    pool1 = MaxPool2D((2, 2),trainable=hparams.frozen)(cnn1)
    cnn2 = Conv2D(128, (3, 3), activation='relu',trainable=hparams.frozen)(pool1)
    pool2 = MaxPool2D((2, 2),trainable=hparams.frozen)(cnn2)
    cnn_out = Flatten()(pool2)


    # protein_similary
    inp_similary = Input(shape=(92120), dtype=np.float32)
    encoded1 = Dense(2048, activation='relu',trainable=hparams.frozen)(inp_similary)
    encoded2 = Dense(1024, activation='relu',trainable=hparams.frozen)(encoded1)
    encoded3 = Dense(512, activation='relu',trainable=hparams.frozen)(encoded2)
    decoded1 = Dense(1024, activation='relu',trainable=hparams.frozen)(encoded3)
    decoded2 = Dense(2048, activation='relu',trainable=hparams.frozen)(decoded1)
    ...
    model.compile(optimizer=params['optimizer'], loss=params['loss'])
    if hparams.load:
        loaded_model = load_model(hparams.origin_path)
        old_weights = loaded_model.get_weights()
        now_weights = model.get_weights()
        cnt = 0
        for i in range(len(old_weights)):
...
# pretrain_model('/public/home/zhengqq/data/model_parma/annopro/0727pre_annopro_bp.h5')
# train_model('/public/home/zhengqq/data/model_parma/annopro/0727pre_annopro_bp.h5', '/public/home/zhengqq/data/Train/cafa5/annopro/time_train_annopro.pkl',
#             valid_path='/public/home/zhengqq/data/Train/cafa5/annopro/time_valid_annopro.pkl', epochs=200, data_size=60000)
# evaluate('0727pre_annopro_bp', '/public/home/zhengqq/data/Train/cafa5/annopro/time_valid_annopro.pkl', data_size=100000, batch_size=32, term_path='/public/home/zhengqq/data/Train/new_terms/new_terms_BP.pkl')
# CRNN_model('/public/home/zhengqq/data/model_parma/annopro/0727annopro_bp.h5',frozen=False,load=True,origin_path='/public/home/zhengqq/data/model_parma/annopro/0725pre_annopro.h5',term_file='/public/home/zhengqq/data/Train/new_terms/new_terms_BP.pkl')
# train_model('/public/home/zhengqq/data/model_parma/annopro/0727annopro_bp.h5','/public/home/zhengqq/data/Train/cafa5/annopro/time_train_annopro.pkl',valid_path='/public/home/zhengqq/data/Train/cafa5/annopro/time_valid_annopro.pkl', batch_size=32,epochs=200)
if __name__ =="__main__":

    parser = ArgumentParser()
    parser.add_argument("--data_path", default='/public/home/zhengqq/data/fanxiu/GB/25train/BP_High_Normal.pkl')
    parser.add_argument("--model_file", default='/public/home/zhengqq/data/fanxiu/model/BP_High_Normal.h5')
    parser.add_argument("--term_file", default='/public/home/zhengqq/data/fanxiu/GB/25terms/BP_High_Normal.pkl')
    parser.add_argument("--valid_path", default='none')
    parser.add_argument("--data_size", default=1000000)
    parser.add_argument("--batch_size", default=32)
    parser.add_argument("--epochs", default=1000)
    parser.add_argument("--early", default=True)
    parser.add_argument("--gpu", default="0")
    parser.add_argument("--frozen", default=False)
    parser.add_argument("--load", default=True)
    parser.add_argument("--origin_path", default=True)
